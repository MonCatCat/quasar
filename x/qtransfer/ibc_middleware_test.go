...
type HooksTestSuite struct {
	...
	chainB      *qtransfertestutils.TestChain
...
func (suite *HooksTestSuite) SetupTest() {
	...
	suite.chainB = &qtransfertestutils.TestChain{
		TestChain: suite.coordinator.GetChain(ibctesting.GetChainID(2)),
	}
	suite.path = NewTransferPath(suite.chainA.TestChain, suite.chainB.TestChain)
...
func (suite *HooksTestSuite) TestOnRecvPacketHooks() {
	...
	testCases := []struct {
		msg      string
		malleate func(*qtransfertestutils.Status)
		expPass  bool
	}{
		{"override", func(status *qtransfertestutils.Status) {
			suite.chainB.GetQuasarApp().TransferStack.
				ICS4Middleware.Hooks = qtransfertestutils.TestRecvOverrideHooks{Status: status}
		}, true},
		{"before and after", func(status *qtransfertestutils.Status) {
			suite.chainB.GetQuasarApp().TransferStack.
				ICS4Middleware.Hooks = qtransfertestutils.TestRecvBeforeAfterHooks{Status: status}
		}, true},
	}
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			path := NewTransferPath(suite.chainA.TestChain, suite.chainB.TestChain)
			...
			if _, ok := suite.chainB.GetQuasarApp().TransferStack.
				ICS4Middleware.Hooks.(qtransfertestutils.TestRecvOverrideHooks); ok {
			...
			if _, ok := suite.chainB.GetQuasarApp().TransferStack.
				ICS4Middleware.Hooks.(qtransfertestutils.TestRecvBeforeAfterHooks); ok {
